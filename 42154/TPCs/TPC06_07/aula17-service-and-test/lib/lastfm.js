...
function searchArtist(artist, cb) {
    ...
    const url = 'http://ws.audioscrobbler.com/2.0/?method=artist.search&artist='+ artist+'&api_key=79b2506be8ce86d852882e1774f1f2e8&format=json' 
    urllib.request(url, (err, data, res)=>{
        if(err) return cb(err)
        const obj = JSON.parse(data)
        if(obj.error) return cb(new Error(obj.message))
        const user = obj.artist.name
        cb(null, user)
    })
}
